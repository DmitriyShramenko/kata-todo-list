...
const App = () => {
  ...
  const onUpdateLabel = (id, newLabel) => {
    setTodoData((prevTodoData) =>
      prevTodoData.map((item) => (item.id === id ? { ...item, label: newLabel, isEditing: false } : item))
    )
  }
  ...
  return (
    <div className="todoapp">
      ...
      <div className="main">
        ...
        />
        <Footer todoLength={itemsLeft} filter={filter} setFilter={setFilter} deleteCompleted={deleteCompleted} />
      </div>
    </div>
  )
}
